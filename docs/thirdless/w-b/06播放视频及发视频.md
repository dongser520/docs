...
export default{
	...
	methods: {
		...
		async chooseVideo(){
			uni.chooseVideo({
				sourceType:['album'],//默认为：['album', 'camera']
				// extension:['mp4'],//根据文件拓展名过滤，默认不过滤。
				compressed:true, //默认值为 true，需要压缩
				maxDuration:60, //最长支持 60 秒
				camera:'back',//'front'、'back' 默认'back'
				success:async (res) => {
					console.log('选择视频res',res);
					if(res.tempFilePath){
						// 发送到服务器或者第三方云存储 同时页面发送中
						// 返回视频地址、封面
						// 页面效果渲染效果
						// 处理封面图
						let poster = '';
