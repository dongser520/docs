...
### ② 在`/components/chat-item-audio/chat-item-audio.vue`中时长显示
```vue
<template>
	<view ...
...
### ③ 在`/components/chat-item/chat-item.vue`中
```vue
<!-- 内容 -->
<view ...
:style="contentStyle"
...
```
### ② 组件 `/pages/chat-item/chat-item.vue`
```vue
<template>
	<view class="px-3">
		<!-- 时间 -->
...
```
### ③ 语音播放组件 `/pages/chat-item-audio/chat-item-audio.vue`
```vue
<template>
