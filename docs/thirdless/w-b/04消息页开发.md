		...
		<view v-for="(item,index) in chatList" :key="index">
			<view class="flex align-stretch">
				...
				position-relative">
					...
					<!-- 角标 -->
					<!-- <text class="bg-danger text-white rounded-circle 
					font-sm position-absolute"
					style="padding-left: 14rpx;padding-right: 14rpx;
					padding-top: 2rpx;padding-bottom: 2rpx;
					right: 15rpx;top: 18rpx;z-index: 100;">9</text> -->
					<u-badge :isDot="false" :value="item.datacount ? item.datacount : 0 " 
					max="999" 
					numberType="overflow" shape="circle" 
					absolute :offset="['18rpx','15rpx']"></u-badge>
					
				</view>
				<!-- 右边 -->
				<view class="flex flex-column border-bottom 
				border-light-secondary flex-1 py-3 pr-3">
					<!-- 上面：昵称 + 时间 -->
					<view class="flex justify-between align-center mb-1">
						<text class="font-md">{{item.nickname}}</text>
						<text class="font-sm text-light-muted">{{item.chat_time}}</text>
