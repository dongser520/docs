...
### 3. 提交加好友申请
注意：<br/>后端在之前的设计中没有 `invite_confirm(是否需要用户确认才能加为好友(0-不需要 1- 需要用户确认 2-其他情况))`字段，如果有同学用的我们本课之前的数据库，那么你可以加上这个字段在数据库中。<br/>另外，记得把`迁移文件`和`模型`都加上这个字段。<br/>或者你可以直接下载本节课课件，把数据库替换一下。<br/>
