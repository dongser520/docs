...
### ③ 游客模式生成一个游客标识
我们以 `/pages/loginCenter/visitor.js` 为例，这个js文件位置可以是任意位置
```js
...
export const registerGuest = async () => {
	...
	const sign = await generateSign({
		deviceId,
		timestamp,
		salt: 'CLIENT_SALT' // 固定盐值（混淆用）
	});
	
	const res = await uni.request({
	    url: '/api/guest/register',
	    method: 'POST',
	    header: { 'X-Security-Sign': sign },
...
#### 2. 在某个页面去执行，如 `/pages/loginCenter/loginCenter.vue`
```vue
<script>
	import { registerGuest } from '@/pages/loginCenter/visitor.js';
	...
	export default {
		...
		methods: {
			async initDevice() {
				try {
					const deviceId = await registerGuest();
					console.log('页面获取的设备ID:', deviceId);
					
					// 显示设备ID到页面上（可选）
					uni.showToast({
						title: `设备ID: ${deviceId.substring(0, 8)}...`,
						icon: 'none',
						duration: 3000
					});
				} catch (e) {
...
## III. 在 `我的` 页面给未登录用户注册一个游客账号
### 1. 设备指纹
在js文件 `/common/js/deviceId.js`
```js
// 引入UUID库 V5
import uuidv5 from '@/common/js/uuidv5.min.js';
...
### 2. 注册游客模式
在 `/pages/loginCenter/visitor.js`
```js
import { getDeviceId } from '@/common/js/deviceId.js';
import configDataJs from '@/common/mixins/configData.js';
	...
	methods: {
		...
		async registerGuest() {
			...
			const sign = await this.generateSign({
				deviceId,
				timestamp,
				salt: this.visitorSalt, // 固定盐值（混淆用）
			});
			*/
			const sign = sha256(
			...
			try {
				const res = await uni.request({
					url: this.requestUrl.http + '/api/chat/visitorRegister',
					method: 'POST',
					header: {
