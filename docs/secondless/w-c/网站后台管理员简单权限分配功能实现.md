...
treeDataSelectClick(keyname,name,id,classname,e){
    ...
    if($(e.target).parent().parent().attr('class') != 'dropdown-submenu'){
        ...
        if(all == len){
            $(e.target).parent().parent().prev().find('input[type="checkbox"]').prop('checked',true).attr('select',1);
        }else{
            $(e.target).parent().parent().prev().find('input[type="checkbox"]').prop('checked',false).attr('select',1);
        }
        // 设置自己的select
        if($(e.target).is(':checked')){
            $(e.target).attr('select',1);
        ...
        //如果一个没选
        if(len == 0){
            $(e.target).parent().parent().prev().find('input[type="checkbox"]').prop('checked',false).attr('select',0);
        }
