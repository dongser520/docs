...
> 
> 解读：
> 1. girl1你通过new Girls();用了原型，girl2你也通过new Girls();用了原型。就是说你的这个Girls实例（内存分配的区间）还是有的，只不过这两块区间（实例）里面是空的;
...
> 4. 我们可以通过图片清晰看到，我们这两块区间，虽然是空的，它是里面有一个__proto__属性，这个属性其实就是prototype，只不过是后面prototype对象的一个指针属性；
...
在搞清楚了上面的概念后，现在判断一个对象实例（对象引用）如girl1，是不是指向了对象Girls的原型对象，基本上，只要实例化了的，它会自动指向的。

### ③ isPrototypeOf()方法：判断一个对象是否指向了该构造函数的原型对象
> ```javascript
> console.log(Girls.prototype.isPrototypeOf(girl1));//true
> let obj = new Object();
> //虽然实例化了，但是后台有没有原型不知道，可以判断
> console.log(Object.prototype.isPrototypeOf(obj));//true
> //说明它只要实例化了，它就默认指向了它的原型对象
> console.log(Object.prototype.isPrototypeOf(girl1));//true
> //原因就是所有原型，构造函数都是Object对象
> 
> //看一下obj没有指向Girls里面的原型对象，很明显没有指向
> console.log(Girls.prototype.isPrototypeOf(obj));//false
> ```
本节课最后我们思考：如果原型里面有属性looks，实例里面也有属性looks，以谁的为准？
